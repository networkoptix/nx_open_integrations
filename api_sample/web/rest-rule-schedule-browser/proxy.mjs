 ...
 * server base URL the PAGE supplied (scheme+host+port), e.g.
 ...
 *
 * METHOD + BODY + BEARER FORWARDING
 * ---------------------------------
 * Unlike a <video src> GET (which can't carry an Authorization header), the rule
 * calls in this sample are normal fetches: the page sends them with their own
 * `Authorization: Bearer <token>` header AND, for PATCH, a JSON body. The proxy
 * forwards the method, the JSON body, and every non-hop-by-hop header (including
 * Authorization) verbatim. Across the relay's cross-host 307 it RE-SENDS the
 * same init (so the bearer + body survive the hop — a browser refuses to do
 * this). For symmetry with the other web samples it ALSO supports a `?auth=`
 * query param fallback (extractAuth) that it converts into a bearer header and
 * strips from the upstream URL, but this client does not need it.
 */
 ...
 * been stripped by the caller, so `subPath` looks like "/rest/v4/...?foo=...".
 *
 * This client sends the token as an Authorization header (not on the URL), but
 * the proxy keeps this conversion for parity with the other web samples (e.g.
 * a token-in-URL case). Exported for the offline tests.
 */
 ...
 * Authorization header (and re-send the method + body — a 307/308 keeps both)
 * across the relay's cross-host 307, the exact thing a browser refuses to do.
 */
async function fetchFollowing(url, init, maxHops = 5) {
  ...
  for (let hop = 0; hop < maxHops; hop++) {
    ...
    if ([301, 302, 303, 307, 308].includes(res.status)) {
      ...
      continue; // same init -> Authorization header + body are re-sent deliberately
    }
    return res;
  }
...
async function proxyTo(base, subPath, req, res, { agent = null } = {}) {
  // The page sends the token as an Authorization header. As a fallback (parity
  // with other samples) also accept ?auth=<token> on the URL and convert it.
  ...
  //
  // Do NOT forward the upstream Content-Length (or Content-Encoding): Node's
  // fetch (undici) transparently DECOMPRESSES a gzip/br response, so the body we
  // pipe below is the decompressed bytes — but the upstream Content-Length is the
  // COMPRESSED size. Forwarding it makes the browser stop reading early and
  // truncate the body (a large rules list arrives as invalid/cut-off JSON, while
  // a tiny login token slips through). Omitting Content-Length lets Node use
  // chunked transfer encoding and send the whole decompressed body.
  const outHeaders = {
    "content-type": upstream.headers.get("content-type") || "application/octet-stream",
  };
  res.writeHead(upstream.status, outHeaders);

  // Pipe the upstream's web ReadableStream straight to the Node response.
  if (upstream.body) {
 ...
 *        self-signed cert: it points at a local server (this is a localhost dev
 *        tool), and such servers normally present one.
...
export function createProxyHandler({ cloudHost = "https://nxvms.com", insecure = false } = {}) {
  ...
  return async function handleProxy(req, res) {
    ...
    // The FIRST segment is the URI-encoded server base URL the PAGE supplied
    // (scheme+host+port); we decode it and forward there.
