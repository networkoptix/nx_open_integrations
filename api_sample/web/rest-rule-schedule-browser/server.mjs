 ...
 *
 * In "Direct to Media Server" mode the user types the server address
 ...
 *
 * Then open the printed URL, pick a mode, log in, list the rules, and set a
 * schedule.
 *
 * Separation of concerns: this file knows about the demo's files; proxy.mjs
 * knows about forwarding API calls (and re-attaching the bearer + re-sending the
 * PATCH body across the relay's 307). They run on one port so the page and its
