 ...
 * headers for it, so a browser blocks a direct call. This module forwards that
 * call from outside the browser (where CORS does not apply).
 *
 ...
 *
 * This sample is CLOUD-ONLY (just a token request), so this proxy handles a
 * single route — there is no site relay and no 307 hop here:
 *
