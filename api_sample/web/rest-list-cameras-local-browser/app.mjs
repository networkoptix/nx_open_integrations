...
/**
 * Wires the form in index.html to NxServerClient. No framework, no build
 * step — a plain ES module loaded with <script type="module">.
 *
 * All the API logic lives in nx-server-client.mjs (and is unit-tested offline).
...
async function onSubmit(event) {
  ...
  const client = new NxServerClient(config);
  try {
    await client.login();
    setStatus("Logged in. Listing cameras…", "info");
