 ...
 *
 * --server-host is the single VMS server this DIRECT sample talks to (include
 * https:// and the port). --insecure makes the proxy accept that server's
 * self-signed TLS certificate, which local lab servers almost always use.
 *
 * Then open the printed URL and enter just your server username + password.
 *
