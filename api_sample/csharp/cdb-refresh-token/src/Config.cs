...
// API logic (TokenSession) and the CLI wiring (Program) so each piece is easy
...
public sealed record AppConfig(
    string? Host, string? User, string? Password, string? MfaCode, string? RefreshToken);
...
public static class Config
{
    private static readonly Dictionary<string, Action<CliArgs, string>> ValueFlags = new()
    {
        ...
        ["--refresh-token"] = (a, v) => a.RefreshToken = v,
        ["--store"] = (a, v) => a.Store = v,
    ...
    {
        ["--force-refresh"] = a => a.ForceRefresh = true,
    ...
    public static AppConfig Resolve(CliArgs args, IReadOnlyDictionary<string, string> envFile)
    {
        ...
        return new AppConfig(
            ...
            RefreshToken: Pick(args.RefreshToken, "NX_CLOUD_REFRESH_TOKEN"));
    }
}
