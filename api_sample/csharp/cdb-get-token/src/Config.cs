...
// Configuration plumbing: a tiny .env reader, a dependency-free argument
// parser, and the CLI > env var > .env precedence rule. Kept apart from the
// API logic (NxCloudTokenClient) and the CLI wiring (Program) so each piece is
// easy to read and to unit-test.

namespace NxGetToken;

/// <summary>Parsed command-line flags.</summary>
public sealed class CliArgs
{
    ...
    public string? CloudSiteId { get; set; }
    public string EnvFile { get; set; } = ".env";
    public bool Insecure { get; set; }
    public bool TokenOnly { get; set; }
}

/// <summary>Resolved configuration (after applying precedence).</summary>
public sealed record AppConfig(
    string? Host, string? User, string? Password, string? MfaCode, string? CloudSiteId);

public static class DotEnv
{
    /// <summary>Read a simple KEY=VALUE .env file. Missing file -> empty map.</summary>
...
public static class Config
{
    private static readonly Dictionary<string, Action<CliArgs, string>> ValueFlags = new()
    {
        ...
        ["--cloud-site-id"] = (a, v) => a.CloudSiteId = v,
    ...
    private static readonly Dictionary<string, Action<CliArgs>> BoolFlags = new()
    {
        ...
        ["--token-only"] = a => a.TokenOnly = true,
    };

    /// <summary>Parse argv. Supports `--flag value`, `--flag=value`, and booleans.</summary>
    ...
    /// <summary>Apply precedence: CLI flag > OS environment variable > .env file.</summary>
    public static AppConfig Resolve(CliArgs args, IReadOnlyDictionary<string, string> envFile)
    {
        ...
        return new AppConfig(
            Host: Pick(args.Host, "NX_CLOUD_HOST"),
            User: Pick(args.User, "NX_CLOUD_USER"),
            Password: Pick(args.Password, "NX_CLOUD_PASSWORD"),
            MfaCode: args.MfaCode, // not read from env, like the other samples
            CloudSiteId: Pick(args.CloudSiteId, "NX_CLOUD_SITE_ID"));
    }
}
