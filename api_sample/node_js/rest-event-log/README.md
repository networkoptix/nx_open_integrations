# REST API — Read the event log via relay (Node.js)

Node.js port of [`../../python/rest-event-log`](../../python/rest-event-log).
Reads ONE site's event history with a **cloud account**, through the Cloud
relay, using the **v4** event endpoint. No third-party dependencies — built-in
...
node rest_event_log.mjs --dotenv ../../.env

# Last 7 days, only motion events, newest first, max 100:
node rest_event_log.mjs --dotenv ../../.env --since 7d --event-type cameraMotionEvent --limit 100

# Discover which event types THIS site exposes, then exit:
node rest_event_log.mjs --dotenv ../../.env --list-event-types

# An absolute window with an existing scoped token:
node rest_event_log.mjs \
...
```bash
node --test test_rest_event_log.mjs   # or: npm test
```
...
| File | Purpose |
|------|---------|
| `rest_event_log.mjs` | The sample (`NxCloudEventLogClient` + parsing helpers + CLI). |
| `test_rest_event_log.mjs` | Offline tests (`node:test`, mocked `fetch`). |
