...
Latest **`/rest/v4`** API. Built-in `fetch` (Node 18+), `node:stream`, and
`node:test` — no build step, zero runtime dependencies. ESM (`.mjs`).
...
Other env vars: `NX_MODE`, `NX_DEVICE_ID`, `NX_MEDIA_FORMAT`.
...
node media_http_stream.mjs --mode direct \
...
node media_http_stream.mjs --mode cloud \
...
# Or put the vars in ../../.env and just pass --dotenv
node media_http_stream.mjs --mode cloud --device-id {camera-id} --dotenv ../../.env
```

Config precedence is **CLI flag > environment variable > `.env`**; credentials are
never hard-coded. The dotenv flag is **`--dotenv`** (not `--env-file`, which Node
20.6+ reserves). If `--out` is omitted, the file is named
...
--dotenv <path>          .env file to read (default .env)
...
```bash
node --test test_media_http_stream.mjs
```

A bare `node --test` won't match `test_*.mjs`, so pass the file explicitly. The
tests cover arg parsing, format/position/duration validation, mode-aware config,
both login flows, media-URL building (live vs. archive, no token in the URL),
streaming to a sink **and** to a real temp file, the relay 307 + bearer re-attach,
and the auth/error paths.
