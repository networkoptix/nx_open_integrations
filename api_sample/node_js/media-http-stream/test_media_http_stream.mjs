...
/**
 * Offline tests for media_http_stream.mjs. No network, no account, no live
 * camera. The "video" is a fake byte stream; saveClip writes it through an
 * injected sink (and, in one test, a real temp file).
 *
 * Run from this folder:  node --test test_media_http_stream.mjs
 */
...
function makeResponse({ status = 200, json = null, text = "", headers = {}, body = null } = {}) {
  const lower = {};
...
function directClient(handler, opts = {}) {
...
function cloudClient(handler, opts = {}) {
  const f = fakeFetch(handler);
...
test("resolveConfig picks CLOUD vars in cloud mode and defaults cloudHost", () => {
  const cfg = resolveConfig(
    ...
    {},
    { NX_CLOUD_USER: "me@x.com", NX_CLOUD_PASSWORD: "pw" },
  );
...
test("missingFields reports what each mode needs", () => {
  ...
  const cloud = resolveConfig({ mode: "cloud" }, {}, {});
...
test("resolveConfig reads NX_MODE from env and .env when no CLI flag", () => {
  const cfg = resolveConfig({ deviceId: "cam1", siteId: SITE }, {}, { NX_MODE: "cloud" });
  assert.equal(cfg.mode, MODE_CLOUD);
});
...
test("cloud login non-OK raises ApiError", async () => {
  const { client } = cloudClient(() => makeResponse({ status: 500, text: "boom" }));
...
test("saveClip raises AuthError on 403 from the media endpoint", async () => {
...
test("logout with no token is a no-op (no fetch)", async () => {
  const { client, f } = directClient(() => makeResponse({ status: 204 }));
  await client.logout();
