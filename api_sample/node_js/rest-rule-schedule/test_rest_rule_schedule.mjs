...
/**
 * Offline tests for rest_rule_schedule.mjs. No network, no account, no real site.
 *
 * Covers: schedule building for every preset (and bad-hour rejection), the
 * schedule summary + rules table, arg parsing (unknown-flag reject), mode-aware config,
 * both login flows (+ 401), listRules (envelope + 403), patchSchedule (PATCH
 * body, empty-200 success, missing id), the relay 307 preserving method + body
 * + bearer, too-many-redirects, and logout.
 *
 * Run from this folder:  node --test test_rest_rule_schedule.mjs
 */
...
test("missingFields reports per-mode requirements", () => {
  assert.deepEqual(missingFields(resolveConfig({ mode: "direct" }, {}, {})).sort(), [
    "password",
    "serverHost",
    "user",
  ]);
  // cloudHost defaults to https://nxvms.com, so it is never "missing".
  assert.deepEqual(missingFields(resolveConfig({ mode: "cloud" }, {}, {})).sort(), [
    "password",
    "siteId",
    "user",
  ]);
