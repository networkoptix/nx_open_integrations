 ...
 *
 * Node.js port of ../../python/cdb-refresh-token. Built-in `fetch` (Node 18+),
 * built-in `node:test`, no third-party dependencies.
 *
...
export function buildPasswordRequest(user, password, mfaCode) {
  const body = {
    grant_type: "password",
    response_type: "token",
    client_id: CLIENT_ID,
...
export class TokenSession {
  ...
  _absorb(data) {
    ...
    const expiresIn = Number.parseInt(
      data.expires_in ?? DEFAULT_EXPIRES_IN_S,
      10,
    );
...
export function short(token) {
  if (!token) return "";
  return token.length > 27 ? `${token.slice(0, 24)}...` : token;
}

function printState(label, sess) {
...
function dump(sess) {
  ...
  process.stderr.write(JSON.stringify(sess.lastRaw, null, 2).slice(0, 4000) + "\n");
