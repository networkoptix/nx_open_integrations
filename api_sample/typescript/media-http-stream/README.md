...
node media_http_stream.ts --mode direct \
...
node media_http_stream.ts --mode cloud \
...
node media_http_stream.ts --mode cloud --device-id {camera-id} --dotenv ../../.env
```
...
reserves). If `--out` is omitted, the file is named
...
```bash
node --test test_media_http_stream.ts
```

29 tests cover arg parsing, format/position/duration validation, mode-aware config,
