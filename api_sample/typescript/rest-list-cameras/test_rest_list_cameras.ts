...
/**
 * Offline tests for rest_list_cameras.ts. No network, no server needed.
 *
 * Run from this folder:  node --test test_rest_list_cameras.ts
 */
...
interface FetchCalls {
  ...
  get: { url: string; headers: Record<string, string> | undefined } | null;
...
type FakeFetch = FetchImpl & { calls: FetchCalls };

/** Serves one queued response per verb; records calls (incl. DELETE count). */
function fakeFetch({ post = null, get = null, del = null }: FakeFetchOptions = {}): FakeFetch {
  const calls: FetchCalls = { post: null, get: null, deleteUrl: null, deleteCalls: 0 };
  const impl = async (url: string | URL | Request, options: RequestInit = {}): Promise<Response> => {
...
function makeClient(fetchOpts: FakeFetchOptions = {}): { client: NxServerClient; f: FakeFetch } {
