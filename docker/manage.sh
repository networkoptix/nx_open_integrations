...
setup_systemd()
{
	if nsenter --mount=/host/proc/1/ns/mnt -- mount | grep /sys/fs/cgroup/systemd >/dev/null 2>&1; 
        then
	       echo "The systemd cgroup hierarchy is already mounted at /sys/fs/cgroup/systemd."
	else
	    if [[ -d /host/sys/fs/cgroup/systemd ]]; 
            then
	...
	echo "The Docker host is now configured for running systemd containers!"
}
...
patch_cloud_host()
{
	local -r FILE=${MEDIASERVER_NETWORK_SO}
	local -r CLOUD_HOST_KEY="this_is_cloud_host_name"
	local -r FILE_PATH_REGEX=".*/\(libnx_network\.\(a\|so\)\|nx_network\.dll\)"
